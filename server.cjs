...
app.post("/api/public/ai-estimator", async (req, res) => {
  ...
  const effectiveOpenAIKey = openaiKey && typeof openaiKey === "string" && openaiKey.trim() ? openaiKey.trim() : process.env.OPENAI_API_KEY;
...
Return strictly valid JSON with this exact schema:
{
  ...
  "estimation": {
    "systemPurpose": "A concise 1-2 sentence purpose statement describing the core objective and value proposition of the proposed system architecture.",
    ...
    "mainFeatures": ["3-5 core functional system features tailored directly to the user's scope and category"],
...
3. **Scalability**: Multi-replica Kubernetes deployment capable of auto-scaling under peak user loads.`,
    estimation: {
      systemPurpose: `To deliver a high-throughput, fault-tolerant ${targetTech || "AI System"} designed specifically for ${industry || "Enterprise"} operations under ${deploymentMode || "Hybrid Cloud"} infrastructure with zero data leakage.`,
      ...
      mainFeatures: [
        `Automated ${targetTech || "Model"} Inference Pipeline with High-Throughput Batch Processing`,
        `Real-time Vector / Data Indexing with Granular Role-Based Data Access (RBAC)`,
        `Air-gapped Zero Data Retention Architecture with End-to-End TLS 1.3 Encryption`,
        `Self-Healing Microservices with Real-time MLOps Drift & Outage Monitoring`
      ],
